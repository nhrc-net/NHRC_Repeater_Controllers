	...
	;;                   from 1.24
	;; NHRC-10 v 1.27 24 December 2015.
	;;		     If voice initial ID is disabled, and
	;;		     hang time is also disabled, do not play
	;;		     initial CW id -- stealth mode.
	;;		     
	;;		     
	;;		     
VER_INT equ	d'1'
...
VER_LO	equ	d'7'
	...
	IFDEF __16F877
	include "p16f877.inc"
	__FUSES _PWRTE_ON & _HS_OSC & _LVP_OFF & _CP_ALL & _WRT_ENABLE_OFF
	ENDIF

	IFDEF __16F877A	
	include "p16f877a.inc"
	__FUSES _PWRTE_ON & _HS_OSC & _LVP_OFF & _CP_ALL
	ENDIF
...
; ******************************************
; ** 25C320 EEPROM COMMANDS AND ADDRESSES ** 
; ******************************************
...
CHUNKED	equ	7		; Kerchunked since last ID
...
CTTUNE	equ	6		; unused courtesy tone
...
; ***************
	...
	;; receiver states
	rx0Stat			; 3d main receiver state
	rx1Stat			; 3e link receiver state
	;; timers
	rx0TOut			; 3f main receiver timeout timer, in seconds
	rx1TOut			; 40 link receiver timeout timer, in seconds
	idTmr			; 41 id timer, in 10 seconds
	hangTmr			; 42 hang timer, in tenths.
	muteTmr			; 43 DTMF muting timer, in tenths.
	lMutTmr			; 44 link muting timer, in tenths.
	ptchTmr			; 45 autopatch timer
	dtATmr			; 46 dtmf access timer
	fanTmr			; 47 fan timer
	tailCtr			; 48 tail message counter.
	unlkTmr			; 49 unlocked mode timer.
	pulsTmr			; 4a pulse timer.
	;; timer presets
	hangDly			; 4b hang timer preset, used enough to keep around
	;; message number
	msgNum			; 4c message number to play
	;; CW generator data
	cwTmr			; 4d CW element timer
	cwByte			; 4e CW current byte (bitmap)
	cwTbTmr			; 4f CW timebase timer
	
	rbTmr			; 50 remote base inactivity timer.
	
	outPort			; 51 output port data buffer
	cTone			; 52 courtesy tone to play

	eeAddrL			; 53 EEPROM address (low byte) to read/write
	eeAddrH			; 54 EEPROM address (low byte) to read/write
	eeCount			; 55 number of bytes to read/write from EEPROM
	;; control operator control flag groups
	group0			; 56 group 0 flags
	group1			; 57 group 1 flags
	group2			; 58 group 2 flags
	group3			; 59 group 3 flags
	group4			; 5a group 4 flags
	group5			; 5b group 5 flags
	group6			; 5c group 6 flags
	group7			; 5d group 7 flags
	group8			; 5e group 8 flags
	group9			; 5f group 9 flags
	mbxFlag			; 60 mailbox flags. must be after last Cntl-op.
	mbxCtl			; 61 mailbox control flags.
	;; ISD control variables
	isdFlag			; 62 isd control flag
	isdRACc			; 63 isd RAC counter.
	isdCmdH			; 64 isd command high  byte.
	isdCmdL			; 65 isd command low   byte.
	isdCmdC			; 66 isd command command byte.
	isdDly			; 67 isd command delay timer...
	isdMsg			; 68 isd message number...
	isdRMsg			; 69 isd RECORD message number...
	;; call restriction initialization control flag
	nnxInit			; 6a call restriction init'n control flag.
	ringTmr			; 6b ring timer.
	ringDur			; 6c ring duration timer.
	ringCtr			; 6d ring count.
	ringPul			; 6e ring pulses counter.
	tuneFlg			; 6f CI-V tune mode control flag.
...
;
DoID
	movf	txFlag,w	; get txFlag - looking to see if rx is still on
...
DoIDInt				; play initial ID
	...
	goto	DoIDICW		; not enabled, play CW.
...
DoIDICW				; play initial CW id.
	btfsc	group0,3	; is hang timer enabled?
	goto	DoIDCW		; send the CW ID
	bcf	flags,initID	; clear initID flag.
	return			; no initial ID -- stealth mode.
...
Start
	...
	movlw	b'00000111'	; set all pins to digital I/O
	movwf	ADCON1
	...
	IFDEF	__16F887
	movlw	b'11110000'	; select RB7-RB4 for interrupt on change
	movwf	IOCB		; set port b pins for interrupt on change
	ENDIF
...
; ********************************************
DebRx
	btfss	tFlags,ONEMS	; ten ms tick, check for receiver active
	goto	NoDeb		; nope
...
Main00				; receiver inactive --> active transition
	...
	;; read kerchunker timer.
	clrw			; default if not setting kerchunk timer

	;; determine if the kerchunk timer should be set
	movf	hangTmr,f	; check hang timer
	btfss	STATUS,Z	; is it zero?
	goto	Main00x		; nope.  don't set kerchunk timer
	
	clrf	eeAddrH		; clear EEPROM high address byte.
	movlw	EEKCTMR		; get EEPROM address of kerchunker filter timer.
	movwf	eeAddrL		; set EEPROM address low byte.
	PAGE3			; select code page 3.
	call	ReadEEw		; read EEPROM.
	PAGE0			; select code page 0.
Main00x
	bsf	STATUS,RP0	; select register page 1.
	movwf	kchkTmr		; set kerchunker timer.
...
Main1				; receiver active state
	...
	btfss	tFlags,HUNDMS	; tenth second tick?
	goto	Main1z		; nope

	bsf	STATUS,RP0	; select register page 1.
	movf	kchkTmr,f	; get kerchunker timer.
	btfsc	STATUS,Z	; is it zero
	goto	Main1z		; yes.  it is zero.
	decf	kchkTmr,f	; decrement the kerchunker timer.
...
Main10d
	;; check the kerchunker timer...
	bsf	STATUS,RP0	; select register page 1.
	movf	kchkTmr,f	; get kerchunker timer.
	btfsc	STATUS,Z	; is it zero
	goto	Main10dd	; yes.  it is zero.
	;; this is a kerchunk event!
	clrf	kchkTmr		; clear the kerchunk timer for next time
	bcf	STATUS,RP0	; select register page 0.
	bsf	flags,CHUNKED	; set kerchunked flag
	goto	ChkRb		; NO courtesy tone, tail, or ID...
	
Main10dd
	bcf	STATUS,RP0	; select register page 0.
	bcf	flags,CHUNKED	; clear kerchunked flag
...
Ck10S				; check 10-second tick flag bit.
	...
	btfss	flags,needID	; is an id needed?
	goto	NeedNoId	; nope.	 Id timer ran out, no ID needed.
	call	DoID		; id timer decremented to zero, play the ID
	goto	NoIDTmr
NeedNoId
	bsf	flags,initID	; the repeater is idle, set the initial ID flag
...
CWSendr
	...
	btfsc	cwByte,7	; is high bit set?
	goto	CWCmd		; yes. this is a command
...
CWNext				; get next character of message
	...
	btfss	cwByte,7	; is high bit set?
	goto	CWNoCmd		; nope.  not a command
CWCmd	
	btfsc	cwByte,6	; bit 6 set?
	goto	CWDone		; yes. end of message.
	btfss	cwByte,5	; bit 5 set indicates pitch command
	goto	CWSRate		; bit 5 clear -- this is a speed command
	
CWPitch				; 
	movf	cwByte,w	; get the cw Byte
	andlw	b'00011111'	; mask off the command bits - rate is left
	addlw	h'10'		; offset into the right part of tone table.
	PAGE3			; select code page 3
	call	GetTone		; get the pitch byte
	PAGE0			; select code page 0
	bsf	STATUS,RP0	; select register page 1
	movwf	cwTone		; save the tone code
	bcf	STATUS,RP0	; select register page 0
	goto	CWNext		; get the next CW byte
	
CWSRate				; set cw rate.
	movf	cwByte,w	; get the cw Byte
	andlw	b'00011111'	; mask off the command bits - rate is left
	PAGE3			; select code page 3
	call	GetCWD		; get the CW delay
	PAGE0			; select code page 0
	bsf	STATUS,RP0	; select register page 1
	movwf	cwRate		; save the rate
	bcf	STATUS,RP0	; select register page 0
	goto	CWNext		; get the next CW byte
...
CWTest
	...
	bsf	PORTB,TONESEL	; set PDC3311 STROBE high
	nop			; short delay (PDC3311 requires 400 ns here)
	bcf	PORTB,TONESEL	; set PDC3311 STROBE low (will be 500 ns pulse)
...
PlayCWx				; finish CW play startup.
	...
	movwf	cwTmr		; preset cw timer
	bsf	STATUS,RP0	; select register page 1
	movlw	h'39'		; default CW tone
	movwf	cwTone		; save it
	movlw	d'60'		; default cw speed 20 WPM
	movwf	cwRate		; save it
	bcf	STATUS,RP0	; select register page 0
...
GetBeep				; get the next beep character
	...
	btfss	outPort,FONECTL	; is phone off hook?
	goto	SetBeep		; no, phone is not off hook, don't change to pause.
...
CIVWrLp				; CIV write to EEPROM checksum check loop.
        call    CIVGetB         ; get the next byte.
...
CWTbl
	...
	retlw	h'31'		; 14 bt
	retlw	h'58'		; 15 sk
	retlw	h'a0'		; 16 magic pitch D#5
	retlw	h'a1'		; 17 magic pitch E5
	retlw	h'a2'		; 18 magic pitch F5
	retlw	h'a3'		; 19 magic pitch F#5
	retlw	h'00'		; 20
	retlw	h'06'		; 21 a
	retlw	h'11'		; 22 b 
	retlw	h'15'		; 23 c
	retlw	h'a4'		; 24 magic pitch G5
	retlw	h'a5'		; 25 magic pitch G#5
	retlw	h'a6'		; 26 magic pitch A5
	retlw	h'a7'		; 27 magic pitch A#5
	retlw	h'a8'		; 28 magic pitch B5
	retlw	h'a9'		; 29 magic pitch C6
	retlw	h'00'		; 30
	retlw	h'09'		; 31 d
	retlw	h'02'		; 32 e
	retlw	h'14'		; 33 f
	retlw	h'aa'		; 34 magic pitch C#6
	retlw	h'ab'		; 35 magic pitch D6
	retlw	h'ac'		; 36 magic pitch D#6
	retlw	h'ad'		; 37 magic pitch E6
	retlw	h'ae'		; 38 magic pitch F6
	retlw	h'af'		; 39 magic pitch F#6
	retlw	h'00'		; 40
	retlw	h'0b'		; 41 g
	retlw	h'10'		; 42 h
	retlw	h'04'		; 43 i
	retlw	h'b0'		; 44 magic pitch G6
	retlw	h'b1'		; 45 magic pitch G#6
	retlw	h'b2'		; 46 magic pitch A6
	retlw	h'b3'		; 47 magic pitch A#6
	retlw	h'b4'		; 48 magic pitch B6
	retlw	h'b5'		; 49 magic pitch C7
	retlw	h'00'		; 50
	retlw	h'1e'		; 51 j
	retlw	h'0d'		; 52 k
	retlw	h'12'		; 53 l
	retlw	h'b6'		; 54 magic pitch C#7
	retlw	h'b7'		; 55 magic pitch D7
	retlw	h'b8'		; 56 magic pitch D#7
	retlw	h'00'		; 57 
	retlw	h'85'		; 58 magic rate  5 WPM
	retlw	h'86'		; 59 magic rate  6 WPM
	retlw	h'00'		; 60
	retlw	h'07'		; 61 m
	retlw	h'05'		; 62 n
 	retlw	h'0f'		; 63 o
	retlw	h'87'		; 64 magic rate  7 WPM
	retlw	h'88'		; 65 magic rate  8 WPM
	retlw	h'89'		; 66 magic rate  9 WPM
	retlw	h'8a'		; 67 magic rate 10 WPM
	retlw	h'8b'		; 68 magic rate 11 WPM
	retlw	h'8c'		; 69 magic rate 12 WPM
	retlw	h'1b'		; 70 q
	retlw	h'16'		; 71 p
	retlw	h'0a'		; 72 r
	retlw	h'08'		; 73 s
	retlw	h'8d'		; 74 magic rate 13 WPM
	retlw	h'8e'		; 75 magic rate 14 WPM
	retlw	h'8f'		; 76 magic rate 15 WPM
	retlw	h'90'		; 77 magic rate 16 WPM
	retlw	h'91'		; 78 magic rate 17 WPM
	retlw	h'92'		; 79 magic rate 18 WPM
	retlw	h'00'		; 80  
	retlw	h'03'		; 81 t
	retlw	h'0c'		; 82 u
	retlw	h'18'		; 83 v
	retlw	h'93'		; 84 magic rate 19 WPM
	retlw	h'94'		; 85 magic rate 20 WPM
	retlw	h'95'		; 86 magic rate 21 WPM
	retlw	h'96'		; 87 magic rate 22 WPM
	retlw	h'97'		; 88 magic rate 23 WPM
	retlw	h'98'		; 89 magic rate 24 WPM
	retlw	h'13'		; 90 z
	retlw	h'0e'		; 91 w
	retlw	h'19'		; 92 x
	retlw	h'1d'		; 93 y
	retlw	h'99'		; 94 magic rate 25 WPM
	retlw	h'9a'		; 95 magic rate 26 WPM
	retlw	h'9b'		; 96 magic rate 27 WPM
	retlw	h'9c'		; 97 magic rate 28 WPM
	retlw	h'9d'		; 98 magic rate 29 WPM
	retlw	h'9e'		; 99 magic rate 30 WPM
	retlw	h'00'		; 100
	retlw	h'00'		; 101
	retlw	h'00'		; 102
	retlw	h'00'		; 103
...
InitTbl
	...
	retlw	d'0'		; 000d remote base auto shutoff timer
	retlw	d'0'		; 000e kerchunker timer 1
...
;	CW magic characters
;	anything with the high-order bit set is "special"
;	0xff = 1111 1111 = EOM
;              100x xxxx = CW speed 0-30 (really 5-30)
;              101x xxxx = CW pitch 0-31 
