...
; Copyright (C) 1996, 1997, 1998, 1999, 2002, Jeffrey B. Otterson, N1KDO,
; and NHRC LLC.
; All Rights Reserved.
;******************************************************************************
...
;
	...
	;__FUSES _CP_OFF & _XT_OSC & _WDT_ON & _LVP_OFF & _MCLRE_ON & _PWRTE_ON
...
PRIO    equ     5		; main receiver has priority
MUTPTTM equ     6		; mute main with PTT drop
MUTPTTR equ     7		; mute RB with PTT drop
...
NoCW
	...
	;; this is part of the new 4-21 drop ptt to mute code.
Unmute
	clrf	muteTmr		; reset the mute timer.
Unmute2
	btfss	cfgFlag,MUTPTTM	; drop ptt to mute enabled?
	goto	Unmute3		; no.
	btfsc	flags,lastTx	; is PTT supposed to be on?
	bsf	PORTB,PTT	; yes, turn PTT back on.
Unmute3
	btfss	txFlag,RXOPEN	; is receiver still unsquelched
	goto	Unmute4		; no.
	btfss	cfgFlag,NOMUTE	; is muting disabled?
	bcf     PORTA,MUTE      ; no, unmute
Unmute4
	btfss	cfgFlag,MUTPTTR	; drop ptt to mute enabled?
	goto	UMEnd		; no.
	btfss	cfgFlag,LINKRPT	; is link port repeater?
	goto	UMEnd		; nope.
	btfsc	flags,lastTx	; is PTT supposed to be on?
	bsf	PORTB,RB_PTT	; yes, turn PTT back on.
UMEnd
	return
	...
	org	0100
Loop1
	movlw	high MainTbl	; in first 1/4 k
...
Main0				; receiver quiet state
	...
	;; this is the squelched->unsquelched transition
	...
	btfss	STATUS,Z	; ok to unmute if zero.
	goto	Main0x		; skip unmute
	bcf     PORTA,MUTE      ; unmute
	btfss	cfgFlag,PRIO	; skip if priority enabled.
	goto	Main0x		; priority not enabled.
	btfss	txFlag,RBOPEN	; priority is enabled, is RB active?
	goto	Main0x		; RB not active.
...
Main1				; receiver active state
	...
	bsf     PORTA,MUTE      ; mute
	btfss	cfgFlag,PRIO	; is priority enabled?
	goto	Main1x		; no.
	btfss	txFlag,RBOPEN	; is RB on?
	goto	Main1x		; no.
...
Main10
	call	Unmute		; adjust muting as required.
Main10a
...
ChkRb
	movlw	high RbTbl	; in second 1/4 k
	movwf	PCLATH		; ensure computed goto stays in range
...
RbRx01
	...
	btfss	cfgFlag,PRIO	; is priority enabled?
	goto	RbRx0u		; no. unmute.
	btfsc	txFlag,RXOPEN	; is main receiver active?
	goto	RbRx0x		; yes.  don't unmute.
RbRx0u	
...
CTBeep
	...
	btfsc	STATUS,Z	; skip if result is not zero? (not .5 sec from hang start)
	goto	CTBeep1		; result is zero, .5 sec from hang start.
	btfss	STATUS,C	; skip if temp >= 0
	goto	CTBeep0		; temp < 5
	
	movf	temp,w		; get temp
	sublw	d'4'		; w = 4 - w
	btfsc	STATUS,C	; skip if w > 4
	goto	CTBeep3		; play tne next segment.
	
CTBeep0				; no tone is wanted at this time
	btfss	txFlag,CWPLAY	; skip 
	bcf	flags,BEEPON	; turn beep off.
	goto	HangNob		; finish hang processing.
	;; at this point, the hang timer has been running for 0.5->1.0 sec

CTBeep1	
	movf	ctSelFl,f	; evaluate ctSelFl
	btfss	STATUS,Z	; skip if it is ZERO.
	goto	CTBeep2		; it is zero.
	clrf	cTone		; there will be no courtesy tone.
	goto	HangNob		; finish hang processing.
...
CTBeep2
	...
	movwf	epradr		; save EEPROM address
	call	ReadEE		; read EEPROM
...
CkTone
	...
	;; this is part of the new 4-21 drop PTT code
	movlw   MUTEDLY		; get mute timer delay
	movwf   muteTmr		; preset mute timer

	btfsc   cfgFlag,NOMUTE  ; check for no muting flag
	goto    MuteE1		; not set, not muting audio...
	bsf     PORTA,MUTE      ; mute

MuteE1	
	btfss	cfgFlag,MUTPTTM	; is drop PTT to mute enabled?
	goto	MuteE2		; not dropping PTT to mute.
	bcf	PORTB,PTT	; turn transmitter off

MuteE2
	btfss	cfgFlag,MUTPTTR	; is drop RB PTT to mute enabled?
	goto	MuteEnd		; not dropping RB PTT to mute.
	bcf	PORTB,RB_PTT	; turn RB transmitter off
...
WrTone				; save tone in EEPROM to init password
	movf    toneCnt,w	; get toneCnt
	sublw   TONES		; w = TONES - w; w now has digit number
	addlw   EETTPRE		; w = w + EETTPRE; the digit's EEPROM address
	movwf   epradr		; EEADR = w
	movf    tone,w		; get tone
	movwf   eprdata		; put into EEPROM data register...
...
Wait
	...
	;; mute timer has timed out.

	;;  this is part of the 4-21 mute changes
	call	Unmute2		; unmute whatever needs it.
...
GetBit				; return mask in w with bit w set
        movlw   high BitTbl	; in range 200-2ff
        movwf   PCLATH          ; ensure that computed goto will stay in range
...
SetHang
	...
	btfsc	STATUS,Z	; is hang preset zero?
	goto	SHang0		; yes.
	movwf   hangTmr		; preset hang timer
	bsf	txFlag,TXHANG	; set hang time transmit flag
	return			; done.

SHang0				; hang timer is zero.
	bcf	txFlag,TXHANG	; clear hang time transmit flag
	return			; done.
...
RxOff                           ; turn off main receiver
	...
	clrf	rxTout		; clear main receiver timeout timer
	btfss	cfgFlag,PRIO	; skip if priority enabled.
	return			; priority not enabled.
	btfss	txFlag,RBOPEN	; skip if RB is active.
	return			; RB not active.
	bcf	PORTB,RB_MUTE	; unmute RB.
        return
...
EEEval
	...
	movlw	high EvalTbl	; in 2nd 1/4 k
	movwf	PCLATH		; ensure computed goto stays in range
	movf	epradr,w	; get the address
	addwf	PCL,f		; computed goto
EvalTbl
...
TstConf
	...
	bsf	PORTA,MUTE	; mute the main receiver
	bsf	PORTB,RB_MUTE	; mute RB.
...
;
GetCwMsg
	movlw   high CWTbl	; this subroutine is in the top 256 bytes
	movwf   PCLATH		; ensure that computed goto will stay in range
