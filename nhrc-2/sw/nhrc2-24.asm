...
; Copyright (C) 2001, 2002, 2003, NHRC LLC.
; All Rights Reserved.
;
...
;
        ...
        include "p16f628.inc"
        __FUSES _CP_ALL & _EXTCLK_OSC & _WDT_OFF & _LVP_OFF & _MCLRE_OFF & _PWRTE_ON
...
; DEBUGGING LEDS
LED0	equ	5		; RA5 is input only.  LED does not work.
LED1	equ	6		; RA6
...
TimrInt
        btfss   flags,BEEP_ON   ;is beep turned on?
...
Start
	...
	movlw	h'10'		; preset port A as muted
	movwf	PORTA           ; init port A
	movlw	h'07'		; clear all comparators.
	movwf	CMCON		;
...
InitISD
        ...
        movwf   epradr          ;set EEPROM address to program 
        call    EEProg          ;program byte
        incf    epradr,f        ;increment address of byte to init
...
Loop
	;; DEBUG
        ;btfss   flags,BEEP_ON   ; is beep turned on?
	;bcf	PORTA,LED1	; beep not on.
        ;btfsc   flags,BEEP_ON   ; is beep turned on?
	;bsf	PORTA,LED1	; beep on.
	;; END DEBUG!
        ;check CW bit
        ...
        btfss   flags,BEEP_ON    ;was "key" down?
        goto    CWKeyUp         ;nope
                                ;key was down        
        bcf     flags,BEEP_ON    ;key->up
...
CWTest
        ...
        movwf   cwTmr           ;preset cw timer
        bsf     flags,BEEP_ON    ;turn key->down
...
Key0
        ...
        bcf     flags,BEEP_ON    ;no, make sure that beep is off.
...
Repeat
        ...
        goto    Repeat1         ;nope...
        ;;
        ;; jeff this is a bug here that should be fixed.
        ;; the timeout timer cannot be disabled.
        ;; this is a shitty thing this way, and should be fixed
        ;; the next time the code is touched.
        ;; 
...
Repeat1
        ...
        goto    CasEnd          ;yes, keep repeating
...
Hang    
        ...
        goto    ISDCtsy
        bsf     flags,BEEP_ON    ;turn on beep
        goto    Hang2
...
Hang1
        ...
        btfss   STATUS,Z        ;zero if equal
        goto    Hang2
        bcf     flags,BEEP_ON    ;turn off beep
...
TimeOut
        btfss   tFlags,TFLCOR   ;is squelch open?
        goto    CasEnd          ;yes, stay timed out...
...
Wait
        ...
        goto    Wait1           ;not running mute timer
        decfsz  muteTmr,f       ;decrement muteDly
        goto    Wait1           ;have not reached the end of the mute time
        btfss   tFlags,TFLCOR   ;is squelch open?
        bcf     PORTA,MUTE      ;yes, unmute

Wait1
	;; check receiver state and process commands.
	;; off->on state transition: clear tone buffer
	;; on->off state transition: evaluate tone buffer
        btfsc   tFlags,TFLCOR   ; is squelch open?
	goto	WCorOff		; yes, squelch is open
	btfss	flags,lastCor	; was it closed last time?
	goto	Loop		; yes.
	;; cor off->on transition.
	bcf	flags,lastCor	; mark as open.
	call	ClrTone		; clear password tones & commands
	goto	Loop		; continue along...

WCorOff
	;; squelch is closed.
	btfsc	flags,lastCor	; was squelch closed last time?
	goto	Loop		; yes.
	;; cor on->off transition.
	bsf	flags,lastCor	; mark as closed.
...
StartCW
        ...
        movwf   cwTmr           ;preset cw timer
        bcf     flags,BEEP_ON   ;make sure that beep is off
...
;        movwf   epradr          ;save address ; jeff 
...
ReadEE
	...
	goto	ReadEE		; no, try to turn interrupts off again.
	bsf	STATUS,RP0	; select bank 1
	movwf	epradr		; save the address...
	movwf	EEADR		; EEADR = w
...
EEProg
	...
	goto	EEProg		; no, try to turn interrupts off again.
        bsf     STATUS,RP0      ; select bank 1
	movf	epradr,w	; w = epradr
	movwf	EEADR		; EEADR = w
	movf	eprdata,w	; w = eprdata
...
; if epradr > EEIEND, return 0.
;
InitDat

        movf    epradr,w        ;get current address
        sublw   EEIEND          ;EEIEND - epradr -> w
        btfss   STATUS,C        ;C is clear if result is negative
...
;
GetCwMsg
        movlw   high MsgTbl     ;this subroutine is in the top 256 bytes
        movwf   PCLATH          ;ensure that computed goto will stay in range
