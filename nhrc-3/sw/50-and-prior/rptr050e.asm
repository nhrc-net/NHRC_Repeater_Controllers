...
ISD_SMA equ     h'15'		; ISD simplex repeater message, alt ID mode
...
XCOR    equ     4               ; COR set at top of loop only from TFLCOR state
CWTICK  equ     5		; cw clock bit
...
DELAY   equ     2               ; delay line is present
...
DeBounc                         ; COR debounce
        ...
        goto    TIntDone        ; yes
	btfsc	cfgFlag,DELAY	; is the audio delay present?
	goto	DCorOff		; yes...
	decfsz	dBounce,f	; no, decrement & test
	goto	TIntDone	; not zero yet
	goto	DBCoff		; it's zero, turn COR off

DCorOff				; delay is present, cor is off
	decfsz	dBounce,f	; is it zero?
	goto	DCor2		; no
	goto	DBCoff		; yes, turn off COR
...
DBCoff
        bsf     tFlags,TFLCOR   ; turn COR off
...
DCorOn				; debounce COR on
	incfsz	dBounce,w	; increment and test for wrap
	goto	DCor1		; not zero
        bcf     tFlags,TFLCOR   ; yes, sum is zero, 255 stored,  turn COR on
	goto	TIntDone	; done with debouncing...
	
DCor1				; debounce COR on not max yet
	incf	dBounce,f	; increment dBounce counter
	goto	TIntDone	; done with debounce...

SCorOn				; SIMPLEX debounce has extra hysteresis
        IF DEBUG == 1
	bcf	tFlags,TFLCOR	; nope, turn on COR flag
	movlw	1		; preset debounce
	movwf	dBounce
	goto	TIntDone
	nop			; waste space so the debug section
	nop			; is the same size as the non-debug
	ELSE
...
NoCW
        movlw   b'11100001'     ; set timer flags mask
	...
	;
        ...
        ;
Loop1
        btfsc   tFlags,TFLCOR   ; is TFLCOR set?
        bsf     tFlags,XCOR     ; set XCOR, once per loop COR
        movlw   h'00'
...
Quiet
        btfsc   tFlags,XCOR     ; is squelch open?
        goto    CasEnd          ; no
	...
	iorwf	sISDTmr,w	; or in backoff timer...
        btfss   STATUS,Z        ; is it zero
	...
	ISDROFF			; stop any recording message
        btfsc   msgNum,MSGREC   ; is record mode already set?
        goto    SimRec          ; yes, record it.
        movlw   ISD_SM          ; ISD simplex message
        btfsc   cfgFlag,ALTID   ; in alternate ID mode?
        movlw   ISD_SMA         ; ISD simplex message, alt id mode
...
KeyUp
	...
	iorwf	sISDTmr,w	; or in backoff timer...
        ...
        btfsc   msgNum,INOTID   ; is it an ID message?
	...
	ISDPOFF			; turn off ISD playback
        ...
        movwf   msgNum          ; set message number
        call    SetMsg		; set up message playback

Key0
	movf	muteTmr,f	; examine mute timer
	btfsc	STATUS,Z	; skip if mute timer is non-zero
        bcf     PORTA,MUTE      ; unmute
        ...
        btfsc   msgNum,MSGREC   ; is record flag set?
	call	Record		; yes... start recording
        goto    CasEnd
...
Repeat1
        btfss   tFlags,XCOR     ; is squelch open?
	...
	clrf	isdPlaH
        ...
        movwf   state           ; change state to quiet
       	btfss	msgNum,ISDSIM	; is this a simplex message?
        goto    RecEnd          ; no
                                ; *** SIMPLEX ***
SimPlay
	call	SetMsg		; prepare to play message...
	bcf	flags,initID	; clear initial ID flag, don't want that here.
        goto    CasEnd
...
RecEnd
	...
	movwf	msgNum		; set message number to play
	call	SetMsg		; set up message playback
        goto    CasEnd          ; done here
...
Hang
        ...
        addlw   2               ; add 200 ms
        btfss   cfgFlag,ISDCTSY ; check for ISD stored courtesy tone
        addlw   3               ; add 300 ms more if not ISD courtesy tone
...
ISDCtsy                         ; want to play ISD message 3 for courtesy tone
	btfsc	msgNum,MSGREC	; is record requested?
	goto	Hang2		; don't play it (it will stomp the record mode)
...
TimeOut
	...
	btfsc	cfgFlag,SIMPLEX	; in simplex mode?
	goto	CasEnd		; surrender quietly if in simplex mode.
        movlw   ISD_TO          ; ISD time out message
        movwf   msgNum          ; set message number
        call    SetMsg		; set up message playback
...
CasE1A				; hi byte of ISD timer found to be zero
	...
	call	RecStop		; stop recording
       	btfss	msgNum,ISDSIM	; is this the simplex message?
	goto	CasE1B		; no.  

	;; jeff -- trouble spot here.  Just play the message RIGHT NOW!

	clrf	tOutTmr		; stop the timeout timer
	clrf	muteTmr
        bsf     PORTA,MUTE      ; mute the audio...
        movlw   STIMOUT
        movwf   state           ; change state to timedout
	call	SetMsg		; prepare to play message...
	bcf	flags,initID	; clear initial ID flag, don't want that here.
        goto    ID1		; continue
	
CasE1B                          ; store message length
        movf    msgNum,w        ; get message number
        andlw   b'00000011'     ; mask out bit 2-7
        addlw   EEM0LEN         ; add address of message 0 length
        movwf   EEADR           ; set EEPROM address
...
CkBkOff
        ...
        call    PlayMsg         ; start the message playing
...
Cmd4x
        ...
        btfss   tBuf1,0		; is it command 41nn
        goto    Cmd40P          ; no, 40nn command
        clrf    sISDTmr         ; clear ISD backoff timer to nuke simplex message.
        bsf     msgNum,MSGREC   ; set message record bit
        goto    Loop

Cmd40P				; command 40nn -- play message
        call    SetMsg		; set up message playback
        goto    Loop
...
Cmd5x
	...
	movf    tBuf1,w         ; get command byte
        ...
        goto    Cmd50Odd        ; nope, 501, 503, etc.
...
Record
	...
	IF NHRC3 == 0		; NHRC-3 target?
                                ; no, this is a NHRC2
        clrf    isdPlaH         ; zero play hi word; useful for all NHRC-2
...
I1420S                          ; ISD1420 simplex
        btfsc   cfgFlag,ALTID   ; alternate ID mode?
        goto    ISD1420A        ; yes
        movlw   D'148'          ; 14.8 seconds
        movwf   isdPlaL         ; save low byte isd max time
	goto	Rec1

I1420A                          ; ISD1420 simplex, alternate ID mode
        movlw   D'198'          ; 19.8 seconds
        movwf   isdPlaL         ; save low byte isd max time
	goto	Rec1

        ELSE                    ; not NHRC-2 target, it's a NHRC-3
...
I2590A  			; ISD2590 simplex, alternate ID mode
...
Rec1				; start recording on the ISD
        call    SelMsg          ; set up the address bits
        bsf     flags,isdRecF   ; set record mode

        ISDRON			; start recording
	IF NHRC3 == 1		; NHRC-3 target?
...
SetMsg				; set up message playback after short delay
	btfss	msgNum,ISDMSG	; is it an ISD message?
	goto	SetOk		; nope, just play it.
        movf    isdPlaL,w       ; fetch lo order byte of ISD play timer
        iorwf   isdPlaH,w	; or in hi order byte of ISD play timer
	iorwf	sISDTmr,w	; or in backoff timer...
        btfsc   STATUS,Z        ; is it zero
	goto	SetOk		; no message is waiting or playing, always play
	;; jeff
	;; if the requested message is an ID, kill the playing message and play
	;; the ID, otherwise, suppress the message to play.
	btfsc	msgNum,ISDSIM	; is the requested message a simplex message?
	return
	btfsc	msgNum,INOTID	; is the requested message NOT an ID?
	return			; NOT an ID, don't interrupt
				; interrupt playing message... 
	ISDPOFF			; stop ISD playback
	clrf	isdPlaL		; clear playback timer, lo byte
	clrf	isdPlaH		; clear playback timer, hi byte
SetOk	
        movlw   ISDBKOF         ; get the message start delay time
        movwf   sISDTmr         ; set the message start delay timer
        call	PTTon		; turn transmitter on
	return
...
PlayMsg
	...
	goto	ISDPlay		; yes, play it now
        btfss   cfgFlag,SIMPLEX ; in simplex mode?
        goto    PlayM1          ; no
        btfss   cfgFlag,ALTID   ; in alternate id mode?
        goto    MsgCWID         ; no, play CWID
...
;
;Play message from ISD1420; message address in msgNum
;
ISDPlay
...
PNorm
	clrf	isdPlaH		; clear hi order byte of ISD play timer
        ...
        call    ReadEE          ; read EEPROM, w has message length after
        movwf   isdPlaL         ; set lo order byte of ISD play timer
        btfss   STATUS,Z        ; is the length 0
	goto	PNow		; no, play...
...
DoIDSim
	...
	iorwf	sISDTmr,w	; or in backoff timer...
        btfss   STATUS,Z        ; is it zero?
	goto	DoIDCW		; no, an ISD message is playing, play CW ID
        btfss   cfgFlag,ALTID   ; in alternate ID mode?
        goto    DoIDCW          ; no. use CW ID.
        btfss   tFlags,XCOR     ; is squelch open?
	goto	DoIDCW		; yes, use CW ID.
        movlw   ISD_IID         ; get ID message 0 (initial ID)
        goto    DoIDany
...
MapDTMF
	...
	org	2138h
	de	h'10'		; 38 -- length of message 0
	de	h'20'		; 39 -- length of message 1
	de	h'30'		; 3a -- length of message 2
	de	h'40'		; 3b -- length of message 3
