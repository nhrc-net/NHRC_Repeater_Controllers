...
;
	...
	ELSE
        ...
        include "p16c84.inc"
	ENDIF
        __FUSES _CP_OFF & _XT_OSC & _WDT_ON
...
NoInit
        ...
        call    GetData         ; read stored configuration from EEPROM
...
CorOn
	...
	btfsc	STATUS,C	; skip if tBuf1 <= EELAST
...
TstEnab
        movf    tBuf1,w         ; get address
        btfss   STATUS,Z        ; check for zero (on/off)
        goto    TstConf         ; non-zero
        movf    tBuf3,f		; get 2nd byte of command
...
TstConf
	...
	goto	TstDone		; no need to mess with the rest...
...
MapDTMF
	...
	org	2100h
        ...
        de	h'00'           ; 01 -- configuration flag
        de	h'03'           ; 02 -- hang timer preset, in tenths
        de	h'1e'           ; 03 -- timeout timer preset, in 1 seconds
        de	h'0a'           ; 04 -- id timer preset, in 10 seconds
	...
	org	2138h
	de	h'03'		; 38 -- length of message 0
	de	h'03'		; 39 -- length of message 1
	de	h'03'		; 3a -- length of message 2
	de	h'03'		; 3b -- length of message 3
	de	h'0a'		; 3c -- password nibble 1
	de	h'0a'		; 3d -- password nibble 2
	de	h'0a'		; 3e -- password nibble 3
	de	h'0a'		; 3f -- password nibble 4
	ENDIF	
        end
