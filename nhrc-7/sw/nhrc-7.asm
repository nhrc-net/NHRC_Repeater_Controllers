        ...
        ;; Copyright 2003, 2004, 2011, NHRC LLC,
        ...
        ;; 7-110.asm 26 February 2004
        ;; 7-111.asm 08 March 2004
        ;; nhrc-7.asm V 1.12 26 Jan 2006
        ;; V 1.13 01 January 2010 -- change debounce timing
        ;; V 1.14 01 January 2010 -- processor support
        ;; V 1.15 25 March 2010 -- fix default ID
        ;; V 1.20 02 January 2011 -- saved state select from CTSEL 1 2 3
        ;;        15 January 2011 -- has priority mode.
        ;; v 1.21 20 April 2011 -- reconfigured factory saved states.
        ;;        02 May 2011 -- fix inquiry of digital outputs
        ;; v 1.22 02 January 2012 -- shorten debounce on saved state select
        ;; v 1.23 08 January 2012 -- fix *71 test courtesy tone works on RX2
        ;;                           fix missing CT for RX2->RX1.
        ;;                           turned off CT for factory states 0,1
        ;; v 1.24 26 January 2016 -- fix outlier unmute problem on port 2
        
VER_INT equ     d'1'            ; remember to change CW version, too.
VER_HI  equ     d'2'            ; search for "VERSION DATA" in the source.
VER_LO  equ     d'4'

LOAD_EE=1
;; TEST_EE causes test data to be loaded into the EEPROM when set to 1
TEST_EE=0
xxSIMULATOR=1
        ...
        __CONFIG _CONFIG1, _PWRTE_ON & _HS_OSC & _LVP_OFF & _CP_OFF & _WDT_OFF
        ELSE
...
RX1PL   equ     6               ; input, RX1 CTCSS present when low.
RX1COR  equ     7               ; input, RX1 COR present when low.
...
; PORTE
DTMF3OE equ     0               ; output, DTMF 3 output enable.
DTMF3DV equ     1               ; input, DTMF digit valid decoder 3 when high.
RE2     equ     2               ; spare.
; *******************
; ** Control Flags **
; *******************
...
DT3RDY  equ     2               ; some sequence received on DTMF-3
...
DT3UNLK equ     7               ; unlocked indicator, control receiver
                ;; 
...
DT3CMD  equ     7               ; received this command from DTMF-3
...
CWNHRC7 equ     h'16'           ; CW: NHRC 7 message.
BP_ALM  equ     h'23'           ; beep: alarm tone
...
; ***************
        ...
        groupa                  ; 6a group a flags
        groupb                  ; 6b group b flags
        groupc                  ; 6c group c flags
        groupd                  ; 6d group d flags
        groupe                  ; 6e group e flags
        groupf                  ; 6f group f flags
        ...
        dt3ptr                  ; 75  DTMF-3 buffer pointer
        dt3tmr                  ; 76  DTMF-3 buffer timer
        rx3flag                 ; 77  rx3flag, for control receiver DTMF-3.
        dtRFlag                 ; 78  DTMF receive flag...
        dtEFlag                 ; 79  DTMF command interpreter control flag
        eebPtr                  ; 7a  eebuf write pointer.
        tFlags                  ; 7b  Timer Flags
        ;; interrupt pseudo-stack to save context during interrupt processing.
        s_copy                  ; 7c  saved STATUS
        p_copy                  ; 7d  saved PCLATH
        ulk3Tmr                 ; 7e  control receiver unlocked mode timer.
        ...
        dt2bufe
        dt2buff
        dt3buf0                 ; DTMF-3 receiver buffer (16 bytes) @ 140
        dt3buf1
        dt3buf2
        dt3buf3
        dt3buf4
        ...
        dt3bufe
        dt3buff
        eebuf00                 ; eeprom write buffer (16 bytes) @ 150
        eebuf01
        eebuf02
        eebuf03
        eebuf04
...
Start
        ...
        movlw   B'00000010'     ; out/out/out/out/out/out/in/out
        movwf   TRISE           ; set port E data direction
        
        movlw   b'00000110'     ; interrupt on CCP1 or TMR2.
...
ClrMem1                         ; clear more memory.
        ...
        clrf    cw1tbtmr        ; CW timebase counter.
...
Start1
        ...
        ;;
        PAGE3                   ; select code page 3.
        movlw   CWNHRC7         ; get controller name announcement.
...
CkRx1                           ; check COR state receiver 1
        btfss   PORTC,RX1COR    ; check cor receiver 1
...
Rx1COR1
        ...
        goto    Rx1On           ; no.
Rx1CkPL                         ; check PL...
        btfsc   PORTC,RX1PL     ; is the PL signal present?
        goto    Rx1Off          ; no.
...
CkDT2L                          ; check for end of LITZ...
        ...
        ;; ******************
CkDTMF3
        btfss   PORTE,DTMF3DV   ; is a DTMF digit being decoded?
        goto    CkDT3L          ; no
        btfsc   rx3flag,LASTDV  ; was it there last time?
        goto    CkDTDone        ; yes, do nothing.
        bsf     rx3flag,LASTDV  ; set last DV indicator.

RdDT3m
        movlw   DTMFDLY         ; get DTMF activity timer preset.
        movwf   dt3tmr          ; set DTMF command timer
        movf    dt3ptr,w        ; get index
        movwf   FSR             ; put it in FSR
        bcf     STATUS,C        ; clear carry (just in case)
        rrf     FSR,f           ; hey! divide by 2.
        movlw   LOW dt3buf0     ; get address of buffer
        addwf   FSR,f           ; add to index.
        bsf     PORTE,DTMF3OE   ; enable DTMF-3 output port.
        movlw   b'00001111'     ; mask bits.
        andwf   PORTA,w         ; get masked bits of tone into W.
        bcf     PORTE,DTMF3OE   ; disable DTMF-3 output port.
        PAGE3                   ; select code page 3.
        call    MapDTMF         ; remap tone into keystroke value..
        PAGE0                   ; select code page 0.
        iorlw   h'0'            ; OR with zero to set status bits.
        bcf     rx3flag,DTMF0   ; clear last zero received.
        btfsc   STATUS,Z        ; was a zero the last received digit?
        bsf     rx3flag,DTMF0   ; yes...
        
        btfsc   dt3ptr,0        ; is this an odd address?
        goto    DT3Odd          ; yes;
        clrf    INDF            ; zero both nibbles.
        movwf   INDF            ; save tone in indirect register.
        swapf   INDF,f          ; move the tone to the high nibble
        goto    DT3Done         ; done here
DT3Odd
        iorwf   INDF,F          ; save tone in low nibble
DT3Done 
        incf    dt3ptr,f        ; increment index
        movlw   h'1f'           ; mask
        andwf   dt3ptr,f        ; don't let index grow past 1f (31)

        goto    CkDTDone        ; done with DTMF checking.

CkDT3L                          ; check for end of LITZ...
        btfss   rx3flag,LASTDV  ; was it low last time?
        goto    CkDTDone        ; yes.  Done.
        bcf     rx3flag,LASTDV  ; clear last DV indicator.
...
CkDt2   
        ...
        goto    CkDt3           ; not zero yet
        bsf     dtRFlag,DT2RDY  ; ready to evaluate command.
CkDt3   
        movf    dt3tmr,f        ; check for zero...
        btfsc   STATUS,Z        ; is it zero
        goto    TonDone         ; yes
        decfsz  dt3tmr,f        ; decrement timer
        goto    TonDone         ; not zero yet
        bsf     dtRFlag,DT3RDY  ; ready to evaluate command.
TonDone
...
PfxDT2
        ...
        goto    PfxDT3          ; no command waiting.
        ;; copy command from DTMF rx buf to command interpreter input buffer.
...
PfxDT3
        ;; evaluate DTMF 3 buffer for command
        btfss   dtRFlag,DT3RDY  ; is a command ready to evaluate?
        goto    XPfxDT          ; no command waiting.
        ;; copy command from DTMF rx buf to command interpreter input buffer.
        movlw   low dt3buf0     ; get address of this DTMF receiver's buffer
        movwf   FSR             ; store.
        movf    dt3ptr,w        ; get command size...
        movwf   cmdSize         ; save it.
        call    CpyDTMF         ; copy the command...
        movf    dt3ptr,w        ; get command size back, CpyDTMF clobbers.
        movwf   cmdSize         ; save it.
        clrf    dt3ptr          ; make ready to receive again.
        clrf    dtEFlag         ; start evaluating from first prefix.
        bsf     dtEFlag,DT3CMD  ; command from DTMF-3
        bsf     dtRFlag,DTSEVAL ; set evaluate DTMF bit.
        btfsc   dtRFlag,DT3UNLK ; port 3 unlocked?
        bsf     dtRFlag,DTUL    ; this port is unlocked.
        bcf     dtRFlag,DT3RDY  ; reset DTMF ready bit.
        goto    DTEval          ; go and evaluate the command right now.
...
LCmdMsg3
        btfss   dtEFlag,DT3CMD  ; was command from DTMF-3 (control rx)?
        return                  ; no, done.
        movwf   temp2           ; save for TX2.
        btfss   group6,4        ; confirm to TX1?
        goto    LCM32
        PAGE3                   ; select code page 3.
        call    PlayCW1         ; start CW, TX1.
        PAGE1                   ; select code page 1.
LCM32
        btfss   group6,5        ; confirm to TX2?
        return                  ; no, done.
        movf    temp2,w         ; get back confirmation message.
        PAGE3                   ; select code page 3.
        call    PlayCW2         ; start CW, TX2.
        PAGE1                   ; select code page 1.
        return
...
UC71P                           ; play courtesy tone.
        ...
        call    PlayCT2         ; play courtesy tone #w
        PAGE2                   ; select code page 2.
        btfss   dtEFlag,DT3CMD  ; is the command from DTMF-3?
        return                  ; no, done.
...
UlCmd70a                        ; 
        ...
        goto    UC70P32         ; no
        PAGE3                   ; select code page 3.
        call    PlayCWe1        ; play CW, TX1.
        PAGE2                   ; select code page 2.
UC70P32
        btfss   group6,5        ; confirm to TX2?
        return                  ; no, done.
        PAGE3                   ; select code page 3.
        call    PlayCWe2        ; play CW, TX2.
        PAGE2                   ; select code page 2.
        return                  ; done.
...
UlMsg3
        btfss   dtEFlag,DT3CMD  ; was command from DTMF-3?
        return                  ; no. done.
        movwf   temp2           ; save for TX2.
...
ULM32
        btfss   group6,5        ; confirm to TX2?
        return                  ; no, done.
        movf    temp2,w         ; get back response message.
        PAGE3                   ; select code page 3.
        call    PlayCW2         ; start CW, TX2.
        PAGE2                   ; select code page 1.
        return
...
InitTbl
        ...
        ;; control operator switches, set 0 -- back to back simplex
        ...
        ;; control operator switches, set 1 -- back to back simplex
        ...
        ;; control operator switches, set 2 -- repeater with remote base
        ...
        ;; control operator switches, set 3 -- two isolated repeaters
        ...
        ;; control operator switches, set 4 -- two repeaters, linked
...
InitTb2
        ...
        retlw   h'23'           ; 00a5 CW ID  6 '7'
        retlw   h'ff'           ; 00a6 CW ID  7 eom
        ...
        retlw   h'23'           ; 00b5 CW ID  6 '7'
        retlw   h'ff'           ; 00b6 CW ID  7 eom
...
MsgTbl
        ...
        retlw   h'00'           ; ' '     -- 1a
        retlw   h'23'           ; '7'     -- 1b 
        retlw   h'00'           ; ' '     -- 1c  search for MORSE to find codes
        retlw   h'18'           ; 'v'     -- 1d  VERSION DATA
        retlw   h'3e'           ; '1'     -- 1e  VERSION DATA
        retlw   h'6a'           ; '.'     -- 1f  VERSION DATA
        retlw   h'3c'           ; '2'     -- 20  VERSION DATA
        retlw   h'38'           ; '3'     -- 21  VERSION DATA
        retlw   h'ff'           ; EOM     -- 22
        retlw   d'05'           ; 23 alarm tone
        retlw   h'b0'           ; 24
        retlw   d'05'           ; 25
        retlw   h'bb'           ; 26
        retlw   d'05'           ; 27
...
DtTbl
        ...
        ;; this is the EEPROM data that is loaded by default
        org     2100h
        ...
        ELSE
        ;;  this is test settings delete later
        ;; JEFF!!!!

        ;; control operator switches, set 0. A->B, B->A. both simplex.
        de      b'10000011'     ; 0010 control operator switches, group 0
        de      b'11001110'     ; 0011 control operator switches, group 1
        de      b'01000001'     ; 0012 control operator switches, group 2
        de      b'11001110'     ; 0013 control operator switches, group 3
        de      b'11101001'     ; 0014 control operator switches, group 4
        de      b'11101001'     ; 0015 control operator switches, group 5
        de      b'00000000'     ; 0016 control operator switches, group 6
        de      b'00000000'     ; 0017 control operator switches, group 7
        de      b'00000000'     ; 0018 control operator switches, group 8
        de      b'11111111'     ; 0019 control operator switches, group 9
        ...
        ;; control operator switches, set 1. A->A
        de      b'01000011'     ; 0020 control operator switches, group 0
        de      b'11001110'     ; 0021 control operator switches, group 1
        de      b'00000001'     ; 0022 control operator switches, group 2
        de      b'11001110'     ; 0023 control operator switches, group 3
        de      b'11101001'     ; 0024 control operator switches, group 4
        de      b'11101001'     ; 0025 control operator switches, group 5
        de      b'00000000'     ; 0026 control operator switches, group 6
        de      b'00000001'     ; 0027 control operator switches, group 7
        de      b'00000000'     ; 0028 control operator switches, group 8
        de      b'11111111'     ; 0029 control operator switches, group 9
        de      h'00'           ; 002a spare
        de      h'00'           ; 002b spare
        ...
        ;; control operator switches, set 2. A->B
        de      b'10000011'     ; 0030 control operator switches, group 0
        de      b'11001110'     ; 0031 control operator switches, group 1
        de      b'00000001'     ; 0032 control operator switches, group 2
        de      b'11001110'     ; 0033 control operator switches, group 3
        de      b'11101011'     ; 0034 control operator switches, group 4
        de      b'11101001'     ; 0035 control operator switches, group 5
        de      b'00000000'     ; 0036 control operator switches, group 6
        de      b'00000010'     ; 0037 control operator switches, group 7
        de      b'00000000'     ; 0038 control operator switches, group 8
        de      b'11111111'     ; 0039 control operator switches, group 9
        de      h'00'           ; 003a spare
        de      h'00'           ; 003b spare
        ...
        ;; control operator switches, set 3. A->A,B
        de      b'11000011'     ; 0040 control operator switches, group 0
        de      b'11001110'     ; 0041 control operator switches, group 1
        de      b'00000001'     ; 0042 control operator switches, group 2
        de      b'11001110'     ; 0043 control operator switches, group 3
        de      b'11101011'     ; 0044 control operator switches, group 4
        de      b'11101011'     ; 0045 control operator switches, group 5
        de      b'00000000'     ; 0046 control operator switches, group 6
        de      b'00000100'     ; 0047 control operator switches, group 7
        de      b'00000000'     ; 0048 control operator switches, group 8
        de      b'11111111'     ; 0049 control operator switches, group 9
        de      h'00'           ; 004a spare
        de      h'00'           ; 004b spare
        ...
        ;; control operator switches, set 4. A->A,B, B->A,B.  Both Repeat
        de      b'11000011'     ; 0050 control operator switches, group 0
        de      b'11001110'     ; 0051 control operator switches, group 1
        de      b'11000001'     ; 0052 control operator switches, group 2
        de      b'11001110'     ; 0053 control operator switches, group 3
        de      b'11101011'     ; 0054 control operator switches, group 4
        de      b'11101011'     ; 0055 control operator switches, group 5
        de      b'00000000'     ; 0056 control operator switches, group 6
        de      b'00001000'     ; 0057 control operator switches, group 7
        de      b'00000000'     ; 0058 control operator switches, group 8
        de      b'11111111'     ; 0059 control operator switches, group 9
        de      h'00'           ; 005a spare
        de      h'00'           ; 005b spare
        ...
        de      h'23'           ; 00a5 CW ID  6 '7'
        de      h'ff'           ; 00a6 CW ID  7 eom
        ...
        de      h'23'           ; 00b5 CW ID  6 '7'
        de      h'ff'           ; 00b6 CW ID  7 eom
        de      h'ff'           ; 00b7 CW ID  8 eom
