	...
	;; Copyright 2000, 2001, 2003, 2004, 2010 NHRC LLC,
	...
	;; 3p-011 28 September 2003
	;; 3p-012 27 July 2004
	;; 3p-013 17 October 2004
	;; 3p-104 16 November 2004
	;; 3p-105 27 November 2004
	;; 3p-106 02 December 2004
	;; 3p-107 01 January 2005
	;; 3p-108 08 March 2005
	;; nhrc-3-plus.asm V 1.10 26 September 2005
	;; nhrc-3-plus.asm V 1.11 01 April 2006
	;; nhrc-3-plus.asm V 1.12 02 May 2006
	;; nhrc-3-plus.asm V 1.13 10 February 2010
	;; nhrc-3-plus.asm V 1.14 15 August 2011
...
;;DEBUG=1				; 
	...
	IFDEF COLORBURST
	__FUSES _PWRTE_ON & _XT_OSC & _LVP_OFF & _WRT_OFF & _CP_ALL
	ELSE
	__FUSES _PWRTE_ON & _HS_OSC & _LVP_OFF & _WRT_OFF & _CP_ALL
	ENDIF
	ENDIF
	...
	__CONFIG _CONFIG1, _PWRTE_ON & _XT_OSC & _LVP_OFF & _CP_OFF & _WDT_OFF
	ELSE
	__CONFIG _CONFIG1, _PWRTE_ON & _XT_OSC & _LVP_OFF & _CP_ON
...
INIT	equ	1		; input, initialize jumper.
CTSEL1	equ	2		; input, courtesy tone selection.
CTSEL2	equ	3		; input, courtesy tone selection.
OUT1	equ	4		; digital output 1.
OUT2	equ	5		; digital output 2.
OUT3	equ	6		; digital output 3.
OUT4	equ	7		; digital output 4.
...
COR0DBS equ	d'20'		; 20 ms. COR on to off debounce time, simplex
...
;B_DIAL	equ	b'00010111'	; beep out phone number from DTMF buffer!
...
CT1	equ	1		; alert mode alerted courtesy tone
CT2	equ	2		; courtesy tone when link tx enabled
CT3	equ	3		; courtesy tone link receiver link tx off
CTSP1	equ	4		; courtesy tone link receiver link tx on
...
CW3PLUS	equ	h'0a'		; CW: NHRC 3 PLUS message.
CW_ON	equ	h'16'		; CW: ON
CW_OFF	equ	h'19'		; CW: OFF
...
; ***************
	...
	;; last var at 0x5e there are 17 left in this block...
...
DoIDETS				; play speech ID for Euro Mode ID
	movlw	VNORMID		; get normal ID message.
	;; end of 1.06 changes...
...
Start
	...
	movlw	B'00001110'	; out/out/out/out/in/in/in/out
	movwf	TRISB		; set port B data direction

	movlw	B'11000000'	; in/in/out/out/out/out/out/out
...
ClrMem
	...
	btfsc	PORTB,INIT	; skip if init button pressed.
...
Start1
	...
	PAGE3			; select code page 3.
	movlw	CW3PLUS		; get NHRC 3 PLUS announcement.
...
CkIPDone			; ISD playback complete.
	...
	bcf	flags,DEF_ID	; clear the deferred ID flag.
	btfss	group1,6	; in simplex mode?
	goto	CkIDone		; no.
	call	ChkID		; yes.	need to ID?
	;; 1.06 change.	 play courtesy tone if enabled.
	btfss	group0,6	; is CT enabled?
	goto	CkIDone		; no.

	movf	PORTB,w		; get port B.
	movwf	temp		; save it.
	rrf	temp,f		; shift right.
	rrf	temp,w		; shift right into w.
	andlw	b'00000011'	; mask out unneeded bits.
...
CkIRDone			; ISD record done.
	...
	andlw	b'00000011'	; restrict message number to reasonable range.
...
CkISRec				; ran out of time recording ISD message.
	goto	CkIDone		; done here.
...
M0ORec
	...
	;; set ISD message address up.
	bcf	PORTC,ISDA0	; reset ISD A0.	 
	bcf	PORTC,ISDA1	; reset ISD A1.
	btfsc	isdRMsg,0	; is bit 0 set?	 
	bsf	PORTC,ISDA0	; set ISD A0.
	btfsc	isdRMsg,1	; is bit 1 set?	 
	bsf	PORTC,ISDA1	; set ISD A1.
	;; start recording...
...
Main00e				;
	;; jeff 1.05 change here, fixes extra ID in euro ID mode
...
NormRec				; normal, 22.3 second message.
	...
	movwf	isdPTlo		; save message length.	
	clrf	isdPThi		; clear play timer hi. 
	goto	SRec		; start recording, up to 22.3 seconds.
	
SimRec
	btfsc	group1,7	; simplex voice ID mode?
	goto	SimRecS		; yes. record shorter message.
...
SimRecS
	...
	movwf	isdPThi		; save hi byte isd max time
	movlw	H'A1'		; 161 more ticks = 67.3 seconds
	movwf	isdPTlo		; save lo byte isd max time
...
Main3Off			; receiver was active, became inactive
	...
	;; now save the message length into EEPROM
	movf	isdRMsg,w	; get message number
	andlw	b'00000011'	; restrict message number to reasonable range.
	addlw	MSG0LEN		; add address of message 0.
	movwf	eeAddr		; set EEPROM address.
...
Ck10mS				; check 10 millisecond timers
	...
	andlw	b'00011110'	; mask leaves 4 digital output control bits.
	xorlw	h'ff'		; invert.
...
HangNob				; hanging without a beep
	...
	call	DoIDETS		; yep.	Play speech ID.
	;; end of 1.06 change
...
LTable				; jump table for locked commands.
	...
	goto	LCmd4		; prefix 04 -- unlock
	goto	LCmd5		; prefix 05 -- reserved
	goto	LCmd6		; prefix 06 -- reserved
	goto	LCmd7		; prefix 07 -- reserved
...
UlCmd01				; set a bit.
	...
	andlw	b'00011110'	; mask only valid outputs.
...
SetDig				; set digital output pins.
	btfss	group5,1	; is OUT1 supposed to be on?
	bcf	PORTB,OUT1	; no
	btfsc	group5,1	; is OUT1 supposed to be off?
	bsf	PORTB,OUT1	; no
	btfss	group5,2	; is OUT2 supposed to be on?
	bcf	PORTB,OUT2	; no
	btfsc	group5,2	; is OUT2 supposed to be off?
	bsf	PORTB,OUT2	; no
	btfss	group5,3	; is OUT3 supposed to be on?
	bcf	PORTB,OUT3	; no
	btfsc	group5,3	; is OUT3 supposed to be off?
	bsf	PORTB,OUT3	; no
	btfss	group5,4	; is OUT4 supposed to be on?
	bcf	PORTB,OUT4	; no
	btfsc	group5,4	; is OUT4 supposed to be off?
	bsf	PORTB,OUT4	; no
	return			; done
...
UlCmd8				; play/record voice messages
	...
	sublw	h'03'		; subtract from max message number.
	btfss	STATUS,C	; skip if result is non-negative.
	goto	UlCmdNG		; bad command.
...
PlaySSp				; entry point for simplex playback.
	bcf	PORTC,ISDA0	; reset ISD A0.	 
	bcf	PORTC,ISDA1	; reset ISD A1.
	btfsc	isdMsg,0	; is bit 0 set?	 
	bsf	PORTC,ISDA0	; set ISD A0.
	btfsc	isdMsg,1	; is bit 1 set?	 
	bsf	PORTC,ISDA1	; set ISD A1.
...
LoadCtlX			; now actually load the data to RAM.
	movlw	EESETSZ		; get the number of bytes to read.
...
InitTbl
	...
	retlw	d'20'		; 0008 cw pitch
	retlw	d'20'		; 0009 cw speed
	...
	retlw	h'3e'		; 0075 CW ID  6 '3'
	retlw	h'00'		; 0076 CW ID  7 ' '
	retlw	h'16'		; 0077 CW ID  8 'P'
	retlw	h'12'		; 0078 CW ID  9 'L'
	retlw	h'0c'		; 0079 CW ID 10 'U'
	retlw	h'08'		; 007a CW ID 11 'S'
	...
	retlw	h'00'		; 00c4 spare
	retlw	h'00'		; 00c5 spare
	retlw	h'00'		; 00c6 spare
	retlw	h'00'		; 00c7 spare
...
CWParms				; set CW parameters from timers.
	...
	de	d'20'		; 0008 cw pitch
	de	d'20'		; 0009 cw speed
	de	d'0'		; 000a spare
	de	d'0'		; 000b spare
	de	d'0'		; 000c spare
	de	d'0'		; 000d spare
	...
	;; control operator switches, set 1
	;; changed at 1.06 so the in-memory set 1 is set up for Euro mode testing.
	de	b'01011011'	; 0020 control operator switches, group 0
	de	b'00001111'	; 0021 control operator switches, group 1
	de	b'00001101'	; 0022 control operator switches, group 2
	...
	de	h'3e'		; 0075 CW ID  6 '3'
	de	h'00'		; 0076 CW ID  7 ' '
	de	h'16'		; 0077 CW ID  8 'P'
	de	h'12'		; 0078 CW ID  9 'L'
	de	h'0C'		; 0079 CW ID 10 'U'
	de	h'08'		; 007a CW ID 11 'S'
	...
	de	h'01'		; 00c0 ISD message 0 length, tenths.
	de	h'01'		; 00c1 ISD message 1 length, tenths.
	de	h'01'		; 00c2 ISD message 2 length, tenths.
	de	h'01'		; 00c3 ISD message 3 length, tenths.
	de	h'00'		; 00c4 spare
	de	h'00'		; 00c5 spare
	de	h'00'		; 00c6 spare
	de	h'00'		; 00c7 spare
