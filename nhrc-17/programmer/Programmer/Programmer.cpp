// Programmer.cpp : NHRC-17 Interoperability Controller 
// Programming Utility
// copyright 2014 NHRC LLC
...
#define COMM_TIMER 1
#define XFER_TIMEOUT 20

#define WORKSTRING_LENGTH 256

#define APP_NAME "NHRC-17 Programmer"
#define INI_FILE_NAME "NHRC-17.ini"
#define COMPORT_CONFIG_NAME "comport"
...
BOOL isBitSet(BYTE byte, int bit);
                     ...
                     int       nCmdShow)
{
	...
	strncpy_s(filter, FILTER_LENGTH, "NHRC-17 Controller Data (*.bin)|*.bin|", _TRUNCATE);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    hWnd = CreateWindow(APP_NAME,             // class name
		                "NHRC-17 Programmer", // Window Title
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
		case WM_CREATE:
			GetPrivateProfileString(APP_NAME, COMPORT_CONFIG_NAME, "COM1:", comport, MAX_COMPORT_LENGTH, INI_FILE_NAME);
			break;
        ...
        case WM_COMMAND:
            ...
            switch (wmId)
            {
				...
				case ID_FILE_RESET_CONTROLLER:
					...
					txBuffer[txBufferCount++] = SC_RESET_CMD_CHAR;
        ...
        case WM_PAINT:
			...
			strncpy_s(text, TEXT_LENGTH,
				"\n1. Connect NHRC-17 to COM port:\n"
				"2. Load data from controller or file.\n"
				"3. Edit Data.\n"
				"4. Save data to controller and/or file", _TRUNCATE);
...
LRESULT CALLBACK GetDataTransferDialogBoxProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_ACTIVATE:
            SetTimer(hDlg, COMM_TIMER, XFER_TIMEOUT, NULL);
            return FALSE;
        ...
        case WM_TIMER:
            ...
            if (ok)
            {
                if (rxBuffer[00] != SC_ATTENTION_CHAR || // response :
                    rxBuffer[01] != SC_WRITE_CMD_CHAR || // response W
                    rxBuffer[02] != addrHiNibble||       // should be same address
                    rxBuffer[03] != addrLoNibble ||      // should be same address
                    rxBuffer[22] != SC_EOM_CHAR)         // should have EOM
                {
					wsprintf(text, "Bad data from controller=\n(\"%s\", %d bytes", rxBuffer, byteCount);
                    ok = FALSE;
                } // if rxBuffer[0] != ...
...
LRESULT CALLBACK SendDataTransferDialogBoxProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_TIMER:
            ...
            {
                if (xferAddress < EEPROM_SIZE)
                    SetTimer(hDlg, COMM_TIMER, 10, NULL);
                else
                {
                    KillTimer(hDlg, COMM_TIMER);
                    PostMessage(hDlg, WM_COMMAND, IDCANCEL, 0);
...
LRESULT CALLBACK CWIDDialogBoxProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_INITDIALOG:
            hwndCtl = GetDlgItem(hDlg, IDC_CWID1TEXT);
            ...
            {
                tempString[i] = cwByte2char(eepromData[EEPROM_CWID1_BASE + i]);
            } // for i 
            SetWindowText(hwndCtl, tempString);

            hwndCtl = GetDlgItem(hDlg, IDC_CWID2TEXT);
            oldWindowProc = (WNDPROC) GetWindowLong(hwndCtl, GWL_WNDPROC);
            SetWindowLong(hwndCtl, GWL_USERDATA, (LONG) oldWindowProc);
            SetWindowLong(hwndCtl, GWL_WNDPROC, (LONG) CWEditProc);
            SendMessage(hwndCtl, EM_LIMITTEXT, CWID_LENGTH, 0);
            ...
            for (i=0; i < CWID_LENGTH; i++)
            {
                tempString[i] = cwByte2char(eepromData[EEPROM_CWID2_BASE + i]);
            } // for i 
            SetWindowText(hwndCtl, tempString);
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                case IDOK:
                    ...
                    for (i=0; i < CWID_LENGTH; i++)
                    {
                        eepromData[EEPROM_CWID1_BASE + i] = char2cwByte(tempString[i]);
                    } // for i
                    ...
                    for (i=0; i < CWID_LENGTH; i++)
                    {
                        eepromData[EEPROM_CWID2_BASE + i] = char2cwByte(tempString[i]);
                    } // for i
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                case IDOK:
					...
					WritePrivateProfileString(APP_NAME, COMPORT_CONFIG_NAME, comport, INI_FILE_NAME);
                    EndDialog(hDlg, LOWORD(wParam));
                    return TRUE;
...
HANDLE OpenCommPort(HWND hwnd)
{
    ...
    commtimeouts.ReadIntervalTimeout = 20;         // ms between characters.  9600 baud is about 1.1 ms  // JEFF
    commtimeouts.ReadTotalTimeoutMultiplier = 10;  // ms per character for total timeout
    commtimeouts.ReadTotalTimeoutConstant = 20;    // ms added to multiplier * numbytes to get total.
    commtimeouts.WriteTotalTimeoutMultiplier = 50; // ms MAX to transmit each character
    commtimeouts.WriteTotalTimeoutConstant = 500;  // ms added to multipler * numbytes to get total.
...
void SetWindowTitle(HWND hwnd, LPSTR name)
{
    ...
    if (name[0] != 0 )
    {
        wsprintf(message, "NHRC-17 Programmer - %s %s", rawFileName, unsaved ? "(unsaved)" : "");
    } // if (filename[0] != 0) 
    else
    {
        wsprintf(message, "NHRC-17 Programmer %s", unsaved ? "(unsaved)" : "");
    } // if (filename[0] != 0) 
