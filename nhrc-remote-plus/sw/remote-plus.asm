...
; 12 January 2004 v 2.0 convert to 8 outputs.
; 12 August 2004 v 2.1 try to fix cw beep by disabling eeprom read interrupt better.
; 18 October 2004 v 2.2 more 8-output bugs fixed
; 29 January 2005 v 2.3 change port numbers from 0-7 to 1-8.
;                       make sure port number is reasonable.
; 24 February 2005 v 2.4 fix bug in port config.
; 10 December 2007 v 2.5 
;*******************************************************************************
VERSION EQU D'025'
...
;
	...
	ELSE
	;; not ICD
	__FUSES _CP_ON & _EXTCLK_OSC & _LVP_OFF & _PWRTE_ON & _MCLRE_OFF 
	ENDIF			; ICD
	ENDIF                   ; 16F628A

        IFDEF __16F628
        include "p16f628.inc"
	__FUSES _CP_ALL & _EXTCLK_OSC & _LVP_OFF & _MCLRE_OFF & _PWRTE_ON
        ENDIF
...
MAXPORT	equ	d'8'		; biggest acceptable port number
...
; A.2 (in ) = DTMF bit 2
; A.3 (in ) = DTMF bit 3
; A.4 (in/out) = init input at start up, beep output after
; A.5 (in ) = DTMF STD/DV digit valid input
; A.6 (out) = PTT enable output
;
;Port A
DTMFQ1  equ     0               ; DTMF decoder output
DTMFQ2  equ     1               ; DTMF decoder output
DTMFQ3  equ     2               ; DTMF decoder output
DTMFQ4  equ     3               ; DTMF decoder output
TMASK   equ     0f              ; mask for DTMF decoder output
INITBIT equ     4
BEEPBIT	equ	4		; beep and init share a.4
DVa     equ     5               ; STD/DV is now on RA5/MCLR\/VPP
PTTa    equ     6               ; PTT is not on RA6/OSC2/CLKOUT
...
Start
	...
	movlw   b'10111111'     ; RA6 is the only output
	movwf   TRISA		; set port port directions
	movlw   b'00000000'     ; RB0-RB7 all output
	movwf   TRISB
...
NoInit
	bsf     STATUS,RP0      ; select bank 1
	movlw   b'10101111'     ; RA6, RA4 are outputs
...
CkDela
	...
	goto	CkTone		; not zero yet...
        movf    pCtrl,w         ; get the port copy
        movwf   PORTB           ; set the port.
...
CkTone1        
	btfss   PORTA,DVa	; check M8870 digit valid
	goto    NoTone		; not set
...
Cmd6x				; command 6x
	...
	goto	BadCmd		; not 60 or 61
       	...
       	movf	tBuf3,w		; get the selected port number
        movwf   temp            ; stash it away.
        movlw   1               ; one!
        subwf   temp,f          ; temp = temp - 1
        btfss   STATUS,C        ; is result positive
        goto    BadCmd          ; not a reasonable value
        movf    temp,w          ; get temp back.
        andlw   b'11111000'     ; mask for invalid digit.
        btfss   STATUS,Z        ; skip if zero
        goto    BadCmd          ; result is greater than 7

        movf    temp,w          ; get the magic value back
...
GetPCW 				; get port number CW character
        ...
        movwf   PCLATH          ; ensure that computed goto will stay in range
        movlw   1               ; one, baby...
	subwf	tBuf3,w		; get the selected bit
	andlw	h'07'		; must be in 0-7 range
	addwf	PCL,f		; computed goto
PCWTbl
	retlw	h'3e'      	; 0 = port 1
...
TstCtrl                         ; evaluate port control byte
	call	ReadEE		; get eeprom data
        ...
        movwf   PORTB           ; set the output port.
        return                  ; done here...
