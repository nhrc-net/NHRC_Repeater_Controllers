	...
	;; 6-110.asm 26 February 2004
        ;; 6-111.asm 08 March 2004
        ;; 6-112.asm 13 November 2006

VER_INT	equ	d'1'		; remember to change CW version, too.
VER_HI	equ	d'1'		; search for "VERSION DATA" in the source.
VER_LO	equ	d'2'
...
; PORTB
DTMF1OE	equ	0		; output, DTMF 0 output enable.
DTMF2OE	equ	1		; output, DTMF 0 output enable.
...
DTMF1DV	equ	2		; input, DTMF digit valid decoder 0 when high.
DTMF2DV	equ	3		; input, DTMF digit valid decoder 1 when high.
...
DT1UNLK equ	5		; unlocked indicator
DT2UNLK equ	6		; unlocked indicator
;NIY	equ	7		; 
		;; 
;dtEFlag -- DTMF command evaluator control flag.
	;; low order 5 bits indicate next prefix number/user command to scan
DT1CMD	equ	5		; received this command from DTMF0
DT2CMD	equ	6		; received this command from DTMF1
;NIY	equ	7		;
...
;	equ	6
RX12SEL	equ	7		; receiver 1/2 select.
...
CWNHRC6	equ	h'16'		; CW: NHRC 6 message.
...
; ***************
	...
	tx1ctd   		; 2f tx1 courtesy tone front porch delay
	...
	tx2ctd   		; 47 tx2 courtesy tone front porch delay
	...
	dt1ptr			; 71  DTMF-0 buffer pointer
	dt1tmr			; 72  DTMF-0 buffer timer
	dt2ptr			; 73  DTMF-1 buffer pointer
	dt2tmr			; 74  DTMF-1 buffer timer
	...
	tFlags			; 79  Timer Flags
	;; interrupt pseudo-stack to save context during interrupt processing.
	s_copy			; 7a saved STATUS
	p_copy			; 7b saved PCLATH
	w_copy			; 7c  saved W register for interrupt handler
	;; room here from 7d to 7f
        endc			; 1st RAM block ends at 7f

	cblock	h'a0'		; 2nd block of RAM at a0h-efh (80 bytes here)
	;; this MUST BE an exact copy of the port 1 variables.
	junk21			; b6 waste of space
	junk22			; b7 waste of space
	junk23			; b8 waste of space
	junk24			; b9 waste of space
	junk25			; ba waste of space
	junk26			; bb waste of space
	junk27			; bc waste of space
	junk28			; bd waste of space
	junk29			; be waste of space
	junk20			; bf waste of space
	endc
	...
	dt1buf0			; DTMF-0 receiver buffer (16 bytes) @ 120
	dt1buf1
	dt1buf2
	dt1buf3
	dt1buf4
	...
	dt1bufe
	dt1buff
	dt2buf0			; DTMF-1 receiver buffer (16 bytes) @ 130
	dt2buf1
	dt2buf2
	dt2buf3
	dt2buf4
...
Start1
	...
	;;
	PAGE3			; select code page 3.
	movlw	CWNHRC6		; get controller name announcement.
...
Rx1OnOff			; receiver was active, became inactive
	...
	movwf	ct1tone		; save the courtesy tone.
        call    SetCT1          ; start the CT front porch timer
...
Rx110a				; look at rx1 into tx2 here.
	...
	movwf	ct2tone		; save the courtesy tone.
        call    SetCT2          ; start the CT front porch timer
...
T2CTDE                          ; done with tx1 ct front porch delay timer
...
NoHang2				; done with hang timers..
	...
	btfss	tx2flag,RX1TX	; is rx1 -> tx2 turned on?
...
NoMutTm1			; done with muting 1 timer...
	...
	btfss	tx2flag,RX2TX	; is rx2 -> tx2 turned on.
...
LTable				; jump table for locked commands.
	...
	goto	LCmd6		; prefix 06 -- invalid command
	goto	LCmd7		; prefix 07 -- invalid command
...
; *************
LCmdMsg
	btfss	dtEFlag,DT1CMD	; was command from rx1?
	goto	LCmdMsg2	; no.
	PAGE3			; select code page 3.
	call	PlayCW1		; start playback
	PAGE1			; select code page 1.
...
LCmdMsg2
	btfss	dtEFlag,DT2CMD	; was command from rx1?
	return
	PAGE3			; select code page 3.
	call	PlayCW2		; start playback
	PAGE1			; select code page 1.
...
UC71P				; play courtesy tone.
	...
	call	PlayCT1		; play courtesy tone #w
	btfsc	dtEFlag,DT2CMD	; is the command from RX1?
	call	PlayCT2		; play courtesy tone #w
	PAGE2			; select code page 2.
	return			; done.
...
PlayCT2				; play a courtesy tone.
	...
	movf	ct2tone,w	; examine ct1tone.
	andlw	h'07'		; force into reasonable range.
	movwf	temp		; copy to temp.
	bcf	STATUS,C	; clear carry bit.
	rlf	temp,f		; multiply msg # by 2
...
InitTbl
	...
	retlw	d'50'		; 0008 tx1 courtesy tone front porch delay
	retlw	d'50'		; 0009 tx2 courtesy tone front porch delay
	retlw	d'20'		; 000a cw pitch tx1
	retlw	d'20'		; 000b cw speed tx1
	retlw	d'20'		; 000c cw pitch tx2
	retlw	d'20'		; 000d cw speed tx2
	retlw	d'0'		; 000e spare
	retlw	d'0'		; 000f spare

	;; control operator switches, set 0
	retlw	b'01000001'	; 0010 control operator switches, group 0
	retlw	b'11000110'	; 0011 control operator switches, group 1
	retlw	b'10000001'	; 0012 control operator switches, group 2
	retlw	b'11000110'	; 0013 control operator switches, group 3
	retlw	b'11001011'	; 0014 control operator switches, group 4
	retlw	b'11001011'	; 0015 control operator switches, group 5
	...
	retlw	b'01000001'	; 0020 control operator switches, group 0
	retlw	b'11000110'	; 0021 control operator switches, group 1
	retlw	b'10000001'	; 0022 control operator switches, group 2
	retlw	b'11000110'	; 0023 control operator switches, group 3
	retlw	b'11001011'	; 0024 control operator switches, group 4
	retlw	b'11001011'	; 0025 control operator switches, group 5
	...
	retlw	b'01000001'	; 0030 control operator switches, group 0
	retlw	b'11000110'	; 0031 control operator switches, group 1
	retlw	b'10000001'	; 0032 control operator switches, group 2
	retlw	b'11000110'	; 0033 control operator switches, group 3
	retlw	b'11001011'	; 0034 control operator switches, group 4
	retlw	b'11001011'	; 0035 control operator switches, group 5
	...
	retlw	b'11001011'	; 0044 control operator switches, group 4
	retlw	b'11001011'	; 0045 control operator switches, group 5
	...
	retlw	b'01000001'	; 0050 control operator switches, group 0
	retlw	b'11000110'	; 0051 control operator switches, group 1
	retlw	b'10000001'	; 0052 control operator switches, group 2
	retlw	b'11000110'	; 0053 control operator switches, group 3
	retlw	b'11001011'	; 0054 control operator switches, group 4
	retlw	b'11001011'	; 0055 control operator switches, group 5
	...
	retlw	h'0a'		; 006c Courtesy Tone 1 04 length seg 3
	retlw	h'15'		; 006d Courtesy Tone 1 05 tone seg 3
	...
	retlw	h'0f'		; 0075 Courtesy tone 2 05 tone seg 3
...
MsgTbl
	...
	retlw	h'3e'		; '1'     -- 20  VERSION DATA
	retlw	h'3c'		; '2'     -- 21  VERSION DATA
	retlw	h'ff'		; EOM     -- 22
	retlw	h'ff'		;         -- 23
	retlw	h'ff'		;         -- 24
	retlw	h'ff'		;         -- 25
...
DtTbl
	...
	de	d'50'		; 0008 tx1 courtesy tone front porch delay
	de	d'50'		; 0009 tx2 courtesy tone front porch delay
	de	d'20'		; 000a cw pitch tx1
	de	d'20'		; 000b cw speed tx1
	de	d'20'		; 000c cw pitch tx2
	de	d'20'		; 000d cw speed tx2
	de	d'0'		; 000e spare
	de	d'0'		; 000f spare

	;; control operator switches, set 0
	de	b'01000001'	; 0010 control operator switches, group 0
	de	b'11000110'	; 0011 control operator switches, group 1
	de	b'10000001'	; 0012 control operator switches, group 2
	de	b'11000110'	; 0013 control operator switches, group 3
	de	b'11101011'	; 0014 control operator switches, group 4
	de	b'11101011'	; 0015 control operator switches, group 5
	...
	;; control operator switches, set 1
	de	b'01000001'	; 0020 control operator switches, group 0
	de	b'11000110'	; 0021 control operator switches, group 1
	de	b'10000001'	; 0022 control operator switches, group 2
	de	b'11000110'	; 0023 control operator switches, group 3
	de	b'11001011'	; 0024 control operator switches, group 4
	de	b'11001011'	; 0025 control operator switches, group 5
	...
	;; control operator switches, set 2
	de	b'01000001'	; 0030 control operator switches, group 0
	de	b'11000110'	; 0031 control operator switches, group 1
	de	b'10000001'	; 0032 control operator switches, group 2
	de	b'11000110'	; 0033 control operator switches, group 3
	de	b'11001011'	; 0034 control operator switches, group 4
	de	b'11001011'	; 0035 control operator switches, group 5
	...
	de	b'11001011'	; 0044 control operator switches, group 4
	de	b'11001011'	; 0045 control operator switches, group 5
	...
	;; control operator switches, set 4
	de	b'01000001'	; 0050 control operator switches, group 0
	de	b'11000110'	; 0051 control operator switches, group 1
	de	b'10000001'	; 0052 control operator switches, group 2
	de	b'11000110'	; 0053 control operator switches, group 3
	de	b'11001011'	; 0054 control operator switches, group 4
	de	b'11001011'	; 0055 control operator switches, group 5
	...
	de	h'0a'		; 006c Courtesy Tone 1 04 length seg 3
	de	h'15'		; 006d Courtesy Tone 1 05 tone seg 3
	...
	de	h'0f'		; 0075 Courtesy tone 2 05 tone seg 3
	...
	de	h'16'		; 0081 Courtesy tone 4 01 tone seg 1
	...
	de	h'3e'		; 00a1 CW ID  2 '1'
	de	h'0d'		; 00a2 CW ID  3 'K'
	de	h'09'		; 00a3 CW ID  4 'D'
	de	h'0f'		; 00a4 CW ID  5 'O'
	de	h'ff'		; 00a5 CW ID  6 eom
	...
	de	h'3e'		; 00b1 CW ID  2 '1'
	de	h'0d'		; 00b2 CW ID  3 'K'
	de	h'09'		; 00b3 CW ID  4 'D'
	de	h'0f'		; 00b4 CW ID  5 'O'
	de	h'ff'		; 00b5 CW ID  6 eom
